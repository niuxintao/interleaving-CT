...
class item:
    def __init__(self):
        ...
        self.wcheck = 0
        self.widen = 0
        self.multif_measure = 0
        self.multif_precise = 0
        self.multif_recall = 0
...
def dealLine(result, strs):
    if strs.find('average') >= 0:
        ...
        elif strs.find('multi_recall')>=0:
            result.multif_recall = float(strlist[1])
        elif strs.find('multi_precise')>=0:
            result.multif_precis = float(strlist[1])
        elif strs.find('multi_f_measure')>=0:
            result.multif_measure = float(strlist[1])
...
# all the masking effects are obtained by the generated test cases which contain multiple MFS, hence we just let the generated test cases to minus multiple, use num_r - multi or numAll - multi
    ...
    print('')
    print('sensity of number of MFS --- multi f-measure')
    ...
    for j in range(0,3):
        ...
        for sub in senMFS:
            app = approach[j]
            filename = 'muti_fmeasure' +'\/'+app+syn+sub+txt
            result = getOneFile(filename)
            print(str(round(result[0].multif_measure,2)) +',', end= '')
        print('],')
